...
> **版本提醒**：两个版本都会从当前仓库本地构建同一份应用镜像。因此，管理后台 UI、导入进度、分批账号加载、批量账号管理与上游错误判定等功能会在两个版本中同步生效；差别仅在于防封版额外使用 WARP、Privoxy 与 FlareSolverr。
...
### 导入数万条 Token 时页面一直显示“正在导入”？
JSON 和 TXT 导入都会显示进度条。达到 `account.refresh.import_async_threshold`（默认 `1000`）时，保存会转为后台任务，进度条按 SQLite 实际写入的批次更新；刷新页面不会取消任务，页面会自动重新连接正在进行的导入。为避免数万条账号同时发起配额探测，超过 `account.refresh.import_immediate_limit`（默认 `500`）时只保存账号，余额会在后续定时调度中同步。账号列表会先显示首批 500 条，并在浏览器后台逐批补齐；进度完成即代表列表数据已全部取回。

### 部分账号显示异常，错误代码为 400？
这不是额度耗尽（额度限制通常是 429）。只有当 Grok/xAI 的 400、401 或 403 响应正文包含“凭据无效、会话找不到、账号被封/撤销”等明确标记时，系统才会将账号标为异常并从调度中排除；普通的请求参数 400 不会被当作失效账号。

### 一键开启 NSFW 大量失败？
上游认证接口会按出口 IP 限流。批量操作默认只处理尚未开启 NSFW 的可用账号，并以低并发分批执行；单个账号遇到 429 会自动退避重试。可在“配置管理 → 批量操作并发”中调整 NSFW 并发、每批账号数、批间等待和重试次数。不要把 NSFW 并发调得过高，否则会再次触发整批限流。
