...
# ---------------------------------------------------------------------------
# Human-readable error explanations for upstream status codes
# ---------------------------------------------------------------------------

_UPSTREAM_STATUS_EXPLAIN: dict[int, str] = {
    400: "请求参数错误（上游拒绝）",
    401: "账号 Token 已过期或无效，请在管理后台删除该账号",
    403: "Cloudflare 反爬拦截（IP/指纹被识别），建议启用防封版或更换代理",
    429: "该账号图片生成额度已用尽，系统已自动尝试换号",
    500: "Grok 服务器内部错误，稍后重试",
    502: "Grok 网关错误，可能正在部署更新，稍后重试",
    503: "Grok 服务暂时不可用，稍后重试",
}


def _explain_upstream_status(status: int | None, body: str = "") -> str:
    """Map an upstream HTTP status code to a human-readable Chinese explanation."""
    if status is None:
        return "上游未返回状态码（网络中断或 TLS 握手失败）"
    base = _UPSTREAM_STATUS_EXPLAIN.get(status, f"上游返回 HTTP {status}")
    if body:
        body_short = body[:200].replace("\n", " ")
        return f"{base} | 上游响应: {body_short}"
    return base
    ...
    *,
    ...
    if blob_b64 is not None:
        ...
        try:
            ...
            # CDN download failed (e.g. assets.grok.com blocked by GFW).
            # Fall back to the original CDN URL instead of crashing the whole request.
            logger.warning(
                "图片CDN下载失败(已降级为远程URL): 原因={} url={}",
                exc, url[:80],
            )
            if fmt == "b64_json":
                raise  # b64 mode needs the raw bytes; can't fall back to url
            return _ImageOutput(api_value=url, markdown_value=f"![image]({url})")
    ...
    *,
    ...
    async with ResettableSession(**kwargs) as session:
        ...
        if response.status_code != 200:
            body = response.content.decode("utf-8", "replace")[:300]
            raise UpstreamError(
                f"图片生成请求被拒: HTTP {response.status_code} — {_explain_upstream_status(response.status_code, body)}",
                status = response.status_code,
                body   = body,
            )
        async for line in response.aiter_lines():
            ...
            async for line in _stream_lite_generate(
                ...
                for ev in adapter.feed(data):
                    ...
                    if ev.kind == "image" and ev.content:
                        ...
                        return image
            raise UpstreamError(
                "图片生成未返回结果 — 可能原因: 内容审核拦截 / 账号图片额度耗尽 / 模型暂不可用，系统已自动换号重试",
                status=503,  # force retry with another account
            )
        except UpstreamError as exc:
            fail_exc = exc
    ...
    *,
    ...
    async def _runner(idx: int) -> None:
        try:
            ...
            errors.append(exc)
            logger.warning(
                "lite image slot failed: model={} slot={}/{} error={}",
                spec.model_name, idx + 1, n, exc,
            )
    ...
    if not collected and errors:
        # All slots failed — raise the first meaningful error with extra context.
        first = errors[0]
        detail = str(first)
        if len(errors) > 1:
            detail += f"（共 {len(errors)} 个并发请求全部失败）"
        if isinstance(first, RateLimitError):
            raise RateLimitError(f"所有可用账号均已耗尽: {detail}") from first
        if isinstance(first, UpstreamError):
            raise first
        raise UpstreamError(f"图片生成失败: {detail}") from first
    return collected
