...
def create_app() -> FastAPI:
    ...
    async def _generic_error_handler(request: Request, exc: Exception):
        # If this is an ExceptionGroup (from asyncio.TaskGroup), extract the
        # first sub-error so the user sees a meaningful message instead of
        # "unhandled errors in a TaskGroup".
        if isinstance(exc, BaseExceptionGroup):
            sub_errors = exc.exceptions
            first = sub_errors[0] if sub_errors else exc
            if isinstance(first, AppError):
                logger.error("task group error: count={} detail={}", len(sub_errors), first.to_dict())
                return JSONResponse(first.to_dict(), status_code=first.status)
            detail = str(first)
            logger.exception("task group error: count={} detail={}", len(sub_errors), detail)
            return JSONResponse(
                {"error": {"message": f"请求处理异常: {detail}", "type": "task_error"}},
                status_code=500,
            )
